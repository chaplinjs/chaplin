...
mediator.setHandler = (name, method, instance) ->
  ...
  handlers[name] = {instance, method: bound}

# Retrieves a handler function.
mediator.getHandler = (name, silent) ->
  handler = handlers[name]
  if handler
    handler.method
  else if not silent
    throw new Error "mediator.getHandler: #{name} handler is not defined"
